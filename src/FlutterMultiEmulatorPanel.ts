...
export class FlutterMultiEmulatorPanel {
  public static currentPanel: FlutterMultiEmulatorPanel | undefined;
  private static readonly viewType = 'flutterMultiEmulator';

  private readonly _panel: vscode.WebviewPanel;
  private readonly _extensionUri: vscode.Uri;
  private readonly _processManager: any;
  private _disposables: vscode.Disposable[] = [];
  private _isPortrait: boolean = true;
  private _currentDevice: string = 'iPhone 14 Pro';
  ...
  public static createOrShow(extensionUri: vscode.Uri, processManager: any): vscode.WebviewPanel | undefined {
    ...
    const panel = vscode.window.createWebviewPanel(
      FlutterMultiEmulatorPanel.viewType,
  ...
  public static dispose() {
    FlutterMultiEmulatorPanel.currentPanel?.dispose();
  ...
  private async _handleScreenshotCapture(imageData: string) {
    try {
      const config = vscode.workspace.getConfiguration('flutterMultiEmulator');
      const screenshotFolder = config.get<string>('screenshotFolder', '${workspaceFolder}/screenshots');
  ...
  private _getHtmlForWebview(webview: vscode.Webview) {
    const config = vscode.workspace.getConfiguration('flutterMultiEmulator');
    const defaultDeviceName = config.get<string>('defaultDevice', 'iPhone 14 Pro');
    ...
    const enableScreenshot = config.get<boolean>('enableScreenshot', true);
